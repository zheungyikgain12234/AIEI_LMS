...
-- `student_type` drives which of (department, role) vs (program_track) is
-- required: an Internal student only picks Department + Role (program_track
-- stays null); an External student only picks a Track (department/role stay
-- null). The check constraint below is the source of truth for that
-- shape — the registration form just mirrors it.
create table students (
  id bigint generated by default as identity primary key,
  name text not null,
  student_code text not null unique,
  email text not null,
  student_type text not null default 'Internal' check (student_type in ('Internal', 'External')),
  department text references departments(name) on update cascade,
  title text,
  program_track text references program_tracks(name) on update cascade,
  -- Not collected at registration — a student's cohort follows from the
  -- class they're later enrolled into, not a standalone choice up front.
  ...
  created_at timestamptz not null default now(),
  constraint students_type_fields_check check (
    (student_type = 'Internal' and department is not null and role is not null and program_track is null)
    or
    (student_type = 'External' and program_track is not null and department is null and role is null)
  )
);
