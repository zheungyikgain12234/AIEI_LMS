...
// ---------------------------------------------------------------------------
// MarkAssignmentScreen — reached via "Mark Assignment" on an assignment
// content block in the Syllabus editor. Shows the class roster, 30 students
// per page, sortable by name/code/status/score; tapping a student opens
// their submission for grading on AssignmentGradingScreen, which can then
// step forward through this same sorted order via "Save and Mark the Next
// Student".
// ---------------------------------------------------------------------------
...
class MarkAssignmentScreen extends StatefulWidget {
  final String contentBlockId;
  final String sectionId;
  final String title;

  const MarkAssignmentScreen({super.key, required this.contentBlockId, required this.sectionId, required this.title});

  @override
  State<MarkAssignmentScreen> createState() => _MarkAssignmentScreenState();
}

class _MarkAssignmentScreenState extends State<MarkAssignmentScreen> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      _maxMarks = criteria.fold<double>(0, (sum, c) => sum + c.maxMarks);
  ...
  /// they're graded, or if the assignment has no criteria/marks set up yet.
          ...
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  Text('Mark Assignment', style: FacultyTypography.headlineMd()),
                  const SizedBox(height: 4),
                  Text('Select a student to grade their submission.', style: FacultyTypography.bodySm()),
