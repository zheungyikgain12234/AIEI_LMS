...
// content block in the Syllabus editor. Shows the class roster with each
// student's submission status; tapping a student opens their submission for
// grading on AssignmentGradingScreen.
// ---------------------------------------------------------------------------
...
class _MarkAssignmentScreenState extends State<MarkAssignmentScreen> {
  ...
  Future<void> _openStudent(RosterStudent s) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => AssignmentGradingScreen(contentBlockId: widget.contentBlockId, studentId: s.studentId, studentName: s.name),
      ),
    );
    _load();
  }
